    ...
    try {
      ...
      if (actualNotifyId) {
        await query(
          ...
          [
            actualNotifyId,
            `${socket.user.anon_tag || "Someone"}: ${body.trim().slice(0, 80)}${body.length > 80 ? "..." : ""}`,
        ...
        io.to(`user:${actualNotifyId}`).emit("notification", {
          type: "new_message",
          title: "💬 New Message",
          body: `${socket.user.anon_tag || "Someone"}: ${body.trim().slice(0, 60)}`,
