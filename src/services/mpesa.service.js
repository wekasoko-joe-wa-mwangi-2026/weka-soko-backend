...
// ── Environment-aware M-Pesa base URL ────────────────────────────────────────
// Set MPESA_ENV=live in Railway to switch from sandbox to production
...
async function handleCallback(body) {
  ...
  if (payment.type === "unlock") {
    await query(
      `UPDATE listings SET is_unlocked = TRUE, unlocked_at = NOW(), status = 'active', is_contact_public = TRUE WHERE id = $1`,
      [payment.listing_id]
    );
    // Notify both parties
