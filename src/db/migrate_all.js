...
async function runMigration() {
  ...
  try {
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS users (
      ...
      name VARCHAR(120) NOT NULL, email VARCHAR(255) UNIQUE NOT NULL,
      password_hash VARCHAR(255) NOT NULL, phone VARCHAR(20),
      role user_role NOT NULL DEFAULT 'buyer', anon_tag VARCHAR(20),
      avatar_url TEXT, is_verified BOOLEAN DEFAULT FALSE,
      email_verify_token VARCHAR(64), email_verify_expires TIMESTAMPTZ,
      is_suspended BOOLEAN DEFAULT FALSE,
      admin_level VARCHAR(20) DEFAULT NULL CHECK (admin_level IN ('viewer','moderator','manager','super')),
      violation_count INT DEFAULT 0, account_status VARCHAR(20) DEFAULT 'active',
      google_id VARCHAR(100), last_seen TIMESTAMPTZ, is_online BOOLEAN DEFAULT FALSE,
      response_rate NUMERIC(5,2) DEFAULT NULL, avg_response_hours NUMERIC(6,2) DEFAULT NULL,
      avg_rating NUMERIC(3,2) DEFAULT NULL, review_count INT DEFAULT 0,
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS listings (
      ...
      title VARCHAR(255) NOT NULL, description TEXT NOT NULL, reason_for_sale TEXT,
      category VARCHAR(80), subcat VARCHAR(80), price NUMERIC(12,2) NOT NULL,
      location VARCHAR(255), county VARCHAR(60), status VARCHAR(30) DEFAULT 'pending_review',
      is_unlocked BOOLEAN DEFAULT FALSE,
      locked_buyer_id UUID REFERENCES users(id) ON DELETE SET NULL,
      locked_at TIMESTAMPTZ, unlocked_at TIMESTAMPTZ, listing_anon_tag VARCHAR(20),
      view_count INT DEFAULT 0, interest_count INT DEFAULT 0,
      expires_at TIMESTAMPTZ DEFAULT NOW() + INTERVAL '75 days',
      expiry_warned BOOLEAN DEFAULT FALSE, moderation_note TEXT,
      reviewed_by UUID REFERENCES users(id) ON DELETE SET NULL, reviewed_at TIMESTAMPTZ,
      search_vector tsvector, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW()
    );`);
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS listing_photos (
      ...
      url TEXT NOT NULL, public_id TEXT, sort_order INT DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW()
    );`);
    await client.query(`CREATE TABLE IF NOT EXISTS listing_reports (
      id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
      listing_id UUID NOT NULL REFERENCES listings(id) ON DELETE CASCADE,
      reporter_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      reason VARCHAR(60) NOT NULL, details TEXT, status VARCHAR(20) DEFAULT 'pending',
      resolved_by UUID REFERENCES users(id), resolved_at TIMESTAMPTZ,
      created_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(listing_id, reporter_id)
    );`);
    await client.query(`CREATE TABLE IF NOT EXISTS password_resets (
      id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
      user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      token VARCHAR(64) NOT NULL UNIQUE, expires_at TIMESTAMPTZ NOT NULL,
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS payments (
      ...
      type VARCHAR(30) NOT NULL DEFAULT 'unlock', amount_kes NUMERIC(12,2) NOT NULL,
      mpesa_phone VARCHAR(20), mpesa_receipt VARCHAR(30), mpesa_checkout_id VARCHAR(100),
      voucher_code VARCHAR(30), status VARCHAR(30) DEFAULT 'pending',
      confirmed_at TIMESTAMPTZ, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW()
    );`);
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS escrows (
      ...
      item_amount NUMERIC(12,2) NOT NULL, fee_amount NUMERIC(12,2) DEFAULT 0,
      total_amount NUMERIC(12,2) NOT NULL, status VARCHAR(30) DEFAULT 'holding',
      admin_approved BOOLEAN DEFAULT FALSE,
      approved_by UUID REFERENCES users(id) ON DELETE SET NULL, approved_at TIMESTAMPTZ,
      buyer_confirmed BOOLEAN DEFAULT FALSE, buyer_confirmed_at TIMESTAMPTZ,
      release_after TIMESTAMPTZ, released_at TIMESTAMPTZ,
      released_by UUID REFERENCES users(id) ON DELETE SET NULL, notes TEXT,
      created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW()
    );`);
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS disputes (
      ...
      raised_by UUID REFERENCES users(id) ON DELETE SET NULL, reason TEXT,
      status VARCHAR(30) DEFAULT 'open',
      resolved_by UUID REFERENCES users(id) ON DELETE SET NULL,
      resolved_at TIMESTAMPTZ, resolution TEXT, created_at TIMESTAMPTZ DEFAULT NOW()
    );`);
    await client.query(`CREATE TABLE IF NOT EXISTS chat_messages (
      ...
      body TEXT NOT NULL, is_blocked BOOLEAN DEFAULT FALSE, block_reason TEXT,
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS chat_violations (
      ...
      reason TEXT, severity VARCHAR(20) DEFAULT 'warning',
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS notifications (
      id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
      user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      type VARCHAR(60), title VARCHAR(255), body TEXT, data JSONB,
      is_read BOOLEAN DEFAULT FALSE, created_at TIMESTAMPTZ DEFAULT NOW()
    );`);
    await client.query(`CREATE TABLE IF NOT EXISTS vouchers (
      id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
      code VARCHAR(30) UNIQUE NOT NULL, type VARCHAR(30) NOT NULL DEFAULT 'unlock',
      discount_percent INT DEFAULT 100, max_uses INT DEFAULT 50, uses INT DEFAULT 0,
      description TEXT, active BOOLEAN DEFAULT TRUE, expires_at TIMESTAMPTZ,
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS reviews (
      ...
      reviewer_role VARCHAR(10) NOT NULL, rating INT NOT NULL CHECK (rating BETWEEN 1 AND 5),
      comment TEXT, is_public BOOLEAN DEFAULT TRUE, created_at TIMESTAMPTZ DEFAULT NOW(),
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS buyer_requests (
      id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
      user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      title VARCHAR(120) NOT NULL, description TEXT NOT NULL,
      budget NUMERIC(12,2), county VARCHAR(60), status VARCHAR(20) DEFAULT 'active',
      created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW()
    );`);
    ...
    await client.query(`CREATE OR REPLACE FUNCTION listings_search_update() RETURNS trigger AS $func$
      BEGIN
        NEW.search_vector := to_tsvector('english', COALESCE(NEW.title,'') || ' ' || COALESCE(NEW.description,'') || ' ' || COALESCE(NEW.category,'') || ' ' || COALESCE(NEW.location,'') || ' ' || COALESCE(NEW.county,''));
        RETURN NEW;
      END $func$ LANGUAGE plpgsql;`).catch(()=>{});
    await client.query(`DROP TRIGGER IF EXISTS listings_search_trigger ON listings`).catch(()=>{});
    await client.query(`CREATE TRIGGER listings_search_trigger BEFORE INSERT OR UPDATE ON listings FOR EACH ROW EXECUTE FUNCTION listings_search_update();`).catch(()=>{});
