  ...
  try {
    ...
    const sql = `
      SELECT
        ...
        l.locked_buyer_id,
        -- Per-listing anonymous identity (never links back to seller account or other listings)
        COALESCE(l.listing_anon_tag, 'Seller_' || upper(substring(md5(l.id::text), 1, 6))) AS seller_anon,
        -- Only reveal real seller details if THIS listing is unlocked
        COALESCE(l.listing_anon_tag, 'Seller_' || upper(substring(md5(l.id::text), 1, 6))) AS seller_anon,
    ...
    try {
      ...
      const result = await withTransaction(async (client) => {
        ...
        const { rows } = await client.query(
          `INSERT INTO listings (seller_id, title, description, reason_for_sale, category, price, location, listing_anon_tag)
           VALUES ($1, $2, $3, $4, $5, $6, $7,
             'Seller_' || upper(substring(md5(gen_random_uuid()::text), 1, 6)))
           RETURNING *`,
