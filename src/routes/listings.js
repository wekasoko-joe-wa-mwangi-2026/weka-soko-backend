...
// ── GET /api/listings/admin/sold ─────────────────────────────────────────────
// Public endpoint for admin to view sold listings
router.get("/admin/sold", async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      ...
      COALESCE((SELECT json_agg(p.url ORDER BY p.sort_order LIMIT 1) FROM listing_photos p WHERE p.listing_id=l.id),'[]'::json) AS photos
      FROM listings l JOIN users u ON u.id=l.seller_id
      WHERE l.status='sold' ORDER BY COALESCE(l.sold_at, l.updated_at) DESC LIMIT $1 OFFSET $2`,
...
// Public endpoint for admin to view all listings with filters
router.get("/admin/all", async (req, res, next) => {
  try {
    const { page = 1, limit = 50, status, search, seller_id } = req.query;
    const offset = (parseInt(page) - 1) * parseInt(limit);
    const params = [];
    const conditions = [];
    ...
    const { rows } = await query(
      `SELECT l.id, l.title, l.category, l.price, l.status, l.county, l.location,
      l.created_at, l.updated_at, l.sold_at, l.sold_channel, l.is_contact_public,
    ...
    res.json({ listings: rows, total: parseInt(cnt[0].count), page: parseInt(page) });
  } catch (err) { next(err); }
});
