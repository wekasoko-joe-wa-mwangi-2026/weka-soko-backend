  ...
  try {
    ...
    const sql = `
      SELECT
        ...
        l.seller_id, l.is_unlocked, l.view_count, l.interest_count, l.created_at, l.expires_at,
  ...
  try {
    ...
    if (scanResult.blocked) {
      return res.status(422).json({
        error: `Field "${scanResult.field}" contains contact info (${scanResult.reason}). Please remove it.`,
...
router.patch("/:id", requireAuth, requireSeller, upload.array("photos", 8), async (req, res, next) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      const uploads = await Promise.all(
        req.files.map((f, i) => uploadBuffer(f.buffer, { folder: `weka-soko/listings/${id}` }).then(r => ({ ...r, sort_order: i + 100 })))
      );
      await Promise.all(uploads.map(({ url, public_id, sort_order }) =>
    ...
    const { rows: fresh } = await query(
      `SELECT l.*, COALESCE((SELECT json_agg(json_build_object('id',p.id,'url',p.url) ORDER BY p.sort_order) FROM listing_photos p WHERE p.listing_id=l.id),'[]'::json) AS photos
       FROM listings l WHERE l.id=$1`, [id]
    );
    res.json(fresh[0]);
  } catch (err) { next(err); }
});
